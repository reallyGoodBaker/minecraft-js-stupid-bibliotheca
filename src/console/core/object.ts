...
export function parseObject(obj: any) {
    ...
    const descs = Reflect.ownKeys(obj).map<[string|symbol, PropertyDescriptor|undefined]>(k => [ k, Object.getOwnPropertyDescriptor(obj, k) ])
    for (const [ k, desc ] of descs) {
        ...
        const tAccessor = createToken(
            'accessor', 'virtual', '', null, root, [], enumerable
        )

        tAccessor.children.push(createToken(
            'key', typeof k, getCtorName(k), k, tAccessor, []
        ))

        if (typeof get === 'function') {
            tAccessor.children.push(createToken(
                'getter', 'function', getCtorName(get), get, tAccessor, []
            ))
        }

        if (typeof set === 'function') {
            tAccessor.children.push(createToken(
                'setter', 'function', getCtorName(set), set, tAccessor, []
            ))
        }
...
export function getCtorName(obj: any) {
...
export function getEnumerable(obj: any, p: string | number | symbol) {
